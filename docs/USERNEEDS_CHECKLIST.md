...
- [ ] Intake runs in compact batches (6-10 prompts max) with adaptive follow-up prompts
...
- [ ] Intake completion returns USER_PROFILE_JSON, COACHING_CONFIG_JSON, and OPEN_QUESTIONS
...
- [ ] Specialist stack includes Goal Strategist + operational Orchestrator + domain specialists
- [ ] Goal Strategist defines 6-24 week strategic targets/phases/pivot rules
- [ ] Orchestrator executes daily/weekly trade-off resolution and priority weighting
- [ ] Safety Clinician retains veto authority over unsafe recommendations
- [ ] Quick mode uses bounded specialist fan-out to control cost
- [ ] Deep mode runs expanded specialist set when needed
- [ ] Optional specialists (Behavior Architect, Recovery & Stress Regulator) activate contextually
- [ ] Coaching responses include proactive success checkpoints tied to user goals
- [ ] Coaching responses include explicit pivot triggers when trends stall
- [ ] Guidance evolves from user history/trend data, not just current prompt text
- [ ] User can enable/disable reminder notifications
- [ ] User can configure reminder interval
- [ ] Browser notification permission flow is explicit
- [ ] Reminder notifications encourage success-oriented check-ins
- [ ] Daily check-in questions are AI-generated by specialists at runtime (role shown per question)
- [ ] Daily check-in wording adapts to time of day (morning/afternoon/evening/late-night)
- [ ] User can cancel daily check-in at any step (button + typed cancel command)
- [ ] Daily check-in can be started on-demand at any time of day
- [ ] Specialist runs validate required data coverage at runtime
- [ ] Missing specialist data yields safe degraded operation (not hard failure)
- [ ] Runtime data/feature gaps auto-create shared feedback entries
- [ ] Runtime gap feedback is deduplicated to prevent spam
--- 
